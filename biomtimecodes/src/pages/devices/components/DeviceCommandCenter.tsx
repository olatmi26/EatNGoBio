...
import type { Device } from "@/mocks/devices";
import type { DeviceCommand } from "@/mocks/deviceDetail";
import ConfirmDialog from "@/components/base/ConfirmDialog";

interface Props {
  device: Device;
  commands: DeviceCommand[];
  onCommandSent: (cmd: DeviceCommand) => void;
}
...
export default function DeviceCommandCenter({ device, commands, onCommandSent }: Props) {
  ...
  const statusBadge = (status: DeviceCommand['status']) => {
    const map = {
      executed: { bg: '#dcfce7', color: '#16a34a', label: 'Executed' },
      pending: { bg: '#fef9c3', color: '#ca8a04', label: 'Pending' },
      failed: { bg: '#fee2e2', color: '#dc2626', label: 'Failed' },
      timeout: { bg: '#f3f4f6', color: '#6b7280', label: 'Timeout' },
    };
    const s = map[status];
