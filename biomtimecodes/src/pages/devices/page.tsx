...
import { mockDeviceList, type Device } from "@/mocks/devices";
...
const emptyForm = {
  name: '', sn: '', ip: '', area: '', timezone: 'Africa/Lagos', transferMode: 'Real-Time', heartbeat: 10,
};
...
export default function DevicesPage() {
  ...
  const [devices, setDevices] = useState<Device[]>(mockDeviceList);
  ...
  const openAdd = () => { setForm({ ...emptyForm }); setEditDevice(null); setShowModal(true); };
  const openEdit = (d: Device) => {
    setForm({ name: d.name, sn: d.sn, ip: d.ip, area: d.area, timezone: d.timezone, transferMode: d.transferMode, heartbeat: d.heartbeat });
    setEditDevice(d);
    setShowModal(true);
  };
  ...
  const handleSave = () => {
    ...
    if (editDevice) {
      ...
      showToast('success', 'Device Updated', `${form.name} has been updated successfully`);
    } else {
      const newDevice: Device = {
        id: `DEV${Date.now()}`, ...form,
        status: 'offline', lastActivity: '-', users: 0, fp: 0, face: 0, firmware: 'Ver 6.60',
      };
      setDevices(prev => [...prev, newDevice]);
      showToast('success', 'Device Added', `${form.name} has been registered`);
    }
    setShowModal(false);
  };
  ...
  const handleDelete = (id: string) => {
    ...
    showToast('success', 'Device Removed', `${d?.name} has been removed`);
  };
    ...
    <DashboardLayout>
      <div className="p-6" style={{ background: bg, minHeight: '100vh' }}>
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold" style={{ color: textPrimary }}>Devices</h1>
            <p className="text-sm mt-0.5" style={{ color: textSecondary }}>
              {devices.length} devices registered · {online} online · No license limits
            </p>
          </div>
          <button
            onClick={openAdd}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {[
            { label: 'Total Devices', value: devices.length, color: textPrimary },
            { label: 'Online', value: online, color: '#16a34a' },
            { label: 'Offline', value: offline, color: '#dc2626' },
            { label: 'Total Users on Devices', value: totalUsers, color: textPrimary },
          ].map((s) => (
            <div key={s.label} className="p-5 rounded-xl" style={{ background: cardBg, border: `1px solid ${border}` }}>
              <p className="text-xs font-medium mb-2" style={{ color: textSecondary }}>{s.label}</p>
              ...
              <tbody>
                {filtered.map((d) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => navigate(`/devices/${d.id}`)} className="text-sm font-medium cursor-pointer hover:underline whitespace-nowrap" style={{ color: '#16a34a' }}>
                        {d.name}
                      </button>
                    </td>
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-center" style={{ color: textPrimary }}>{d.users}</td>
                    <td className="px-4 py-3 text-sm font-medium text-center" style={{ color: textPrimary }}>{d.fp}</td>
                    <td className="px-4 py-3 text-sm font-medium text-center" style={{ color: textPrimary }}>{d.face}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => navigate(`/devices/${d.id}`)} className="w-7 h-7 flex items-center justify-center rounded-lg cursor-pointer transition-colors" style={{ color: textSecondary }} title="View Details"
                        ...
                        <button onClick={() => { navigate(`/devices/${d.id}`); }} className="w-7 h-7 flex items-center justify-center rounded-lg cursor-pointer transition-colors" style={{ color: '#7c3aed' }} title="Command Center"
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. LEKKI BIOMETRICS" className={inputClass} style={inputStyle} />
          </div>
          <div>
            <label style={labelStyle}>Serial Number <span style={{ color: '#dc2626' }}>*</span></label>
            <input value={form.sn} onChange={(e) => setForm(f => ({ ...f, sn: e.target.value }))} placeholder="e.g. BQC2254800XXX" className={inputClass} style={inputStyle} />
          </div>
          <div>
            <label style={labelStyle}>Device IP <span style={{ color: '#dc2626' }}>*</span></label>
            <input value={form.ip} onChange={(e) => setForm(f => ({ ...f, ip: e.target.value }))} placeholder="e.g. 192.168.1.199" className={inputClass} style={inputStyle} />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="number" value={form.heartbeat} onChange={(e) => setForm(f => ({ ...f, heartbeat: Number(e.target.value) }))} className={inputClass} style={inputStyle} />
          </div>
          <div>
            <label style={labelStyle}>Attendance Device</label>
            <select className={inputClass} style={inputStyle} defaultValue="Yes">
              <option>Yes</option>
              <option>No</option>
            </select>
          </div>
