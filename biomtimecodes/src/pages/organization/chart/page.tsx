...
import { mockDepartments, mockPositions, mockAreas, type Department, type Position, type Area } from "@/mocks/organization";
...
export default function OrgChartPage() {
  ...
  const totalEmployees = mockDepartments.reduce((s, d) => s + d.employeeQty, 0);
  const totalPositions = mockPositions.reduce((s, p) => s + p.employeeQty, 0);
    ...
    <DashboardLayout>
      <div className="p-4 md:p-6" style={{ background: bg, minHeight: "100vh" }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
          {[
            { label: "Departments", value: mockDepartments.length, icon: "ri-building-2-line", color: "#16a34a", bg: "#dcfce7" },
            { label: "Positions", value: mockPositions.length, icon: "ri-briefcase-line", color: "#0891b2", bg: "#e0f2fe" },
          ...
          <div className="rounded-xl overflow-hidden" style={{ background: cardBg, border: `1px solid ${border}` }}>
            <div className="px-5 py-3.5 flex items-center justify-between" style={{ borderBottom: `1px solid ${border}` }}>
              ...
              <span className="text-xs px-2.5 py-1 rounded-full" style={{ background: "#dcfce7", color: "#16a34a" }}>
                {mockDepartments.length} departments · {totalEmployees} employees
              </span>
            </div>
            ...
            <div className="p-6 overflow-x-auto">
              <div className="flex flex-col items-center gap-6 min-w-max">
                {(search ? filteredDepts.map(d => ({
                  ...
                  children: [], color: getDeptColor(mockDepartments.indexOf(d)), depth: 0,
                })) : tree).map((node, i) => (
            ...
            <div style={{ borderTop: `1px solid ${border}` }}>
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[600px]">
                  ...
                  <tbody>
                    {(search ? filteredDepts : mockDepartments).map((d, i) => (
          ...
          <div className="rounded-xl overflow-hidden" style={{ background: cardBg, border: `1px solid ${border}` }}>
            <div className="px-5 py-3.5 flex items-center justify-between" style={{ borderBottom: `1px solid ${border}` }}>
              <div>
                ...
                <p className="text-xs mt-0.5" style={{ color: textSecondary }}>{mockPositions.length} positions · {totalPositions} total employees</p>
              </div>
            </div>
            ...
            <div className="p-5 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-3 mb-0">
              {filteredPositions.map((pos, i) => {
                ...
                const maxQty = Math.max(...mockPositions.map(p => p.employeeQty));
                return (
                  <button
                    key={pos.id}
