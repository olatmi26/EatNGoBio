...
import { mockShifts, mockShiftAssignments, type Shift, type ShiftAssignment } from "@/mocks/shifts";
...
export default function ShiftsPage() {
  ...
  const handleAssign = (assignment: ShiftAssignment) => {
  ...
  return (
    <DashboardLayout>
      <div className="p-6" style={{ background: bg, minHeight: '100vh' }}>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="relative">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search..." className="pl-9 pr-4 py-2 rounded-xl text-sm outline-none w-52"
                    ...
                    {shift.breaks.length > 0 && (
                      <div className="mb-3 space-y-1">
                        {shift.breaks.map(brk => (
                          <div key={brk.id} className="flex items-center justify-between px-2.5 py-1.5 rounded-lg text-xs" style={{ background: isDark ? '#374151' : '#f0fdf4' }}>
                            <span style={{ color: textPrimary }}>{brk.name}</span>
                            <span style={{ color: textSecondary }}>{brk.startTime}–{brk.endTime} {brk.paid ? '(Paid)' : '(Unpaid)'}</span>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between pt-3" style={{ borderTop: `1px solid ${border}` }}>
                      ...
                      <div className="flex flex-wrap gap-1 justify-end">
                        {shift.locations.slice(0, 2).map(loc => (
                          <span key={loc} className="text-xs px-2 py-0.5 rounded-full" style={{ background: isDark ? '#374151' : '#f3f4f6', color: textSecondary }}>{loc}</span>
                        ))}
                        {shift.locations.length > 2 && (
                          <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: isDark ? '#374151' : '#f3f4f6', color: textSecondary }}>+{shift.locations.length - 2}</span>
                        )}
                      </div>
                    </div>
                  </div>
                ...
                {filteredAssignments.map(a => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-xs whitespace-nowrap" style={{ color: textSecondary }}>{a.endDate || <span style={{ color: '#16a34a' }}>Ongoing</span>}</td>
      ...
      <ShiftModal open={showShiftModal} onClose={() => setShowShiftModal(false)} onSave={handleSaveShift} editShift={editShift} />
      <AssignShiftModal open={showAssignModal} onClose={() => setShowAssignModal(false)} onSave={handleAssign} />
      <ConfirmDialog open={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={() => deleteId && handleDeleteShift(deleteId)} title="Delete Shift" message="Are you sure you want to delete this shift? All assignments will be affected." confirmLabel="Delete" danger />
    </DashboardLayout>
  );
}
