...
import { mockPunchLogs } from "@/mocks/deviceDetail";
import type { PunchLog } from "@/mocks/deviceDetail";

const NAMES = ["Kuzunwhe Adeyemi", "Taiwo Hassan", "Odunayo Okafor", "Godwin Eze", "Adaeze Nwosu", "Emeka Obi", "Fatima Bello", "Babatunde Lawal", "Ngozi Amadi", "Chukwuemeka Eze", "Amina Yusuf", "Halima Musa", "Tunde Fashola", "Oluwaseun Adeyemi", "Chioma Okafor", "Ibrahim Suleiman", "Blessing Nwachukwu", "Segun Ogundimu", "Yetunde Afolabi", "Musa Abdullahi"];
const DEPARTMENTS = ["HEAD-OFFICE", "Operations", "Kitchen", "Security", "Finance", "HR and Admin", "IT", "Logistics", "Procurement"];
const PUNCH_TYPES = ["fingerprint", "face", "fingerprint", "fingerprint", "face"] as const;
const VERIFY_MODES = ["Check-In", "Check-Out", "Check-In", "Check-In", "Break", "Return"] as const;

function generateLivePunch(): PunchLog & { deviceName: string } {
  const now = new Date();
  const name = NAMES[Math.floor(Math.random() * NAMES.length)];
  const device = mockDeviceList[Math.floor(Math.random() * mockDeviceList.length)];
  const punchType = PUNCH_TYPES[Math.floor(Math.random() * PUNCH_TYPES.length)];
  const verifyMode = VERIFY_MODES[Math.floor(Math.random() * VERIFY_MODES.length)];
  return {
    id: `live-${Date.now()}-${Math.random()}`,
    employeeId: String(Math.floor(10000 + Math.random() * 90000)),
    employeeName: name,
...
type ExtendedPunch = PunchLog & { deviceName: string };

export default function LiveMonitorPage() {
  const { isDark } = useTheme();
  const navigate = useNavigate();
  const [punches, setPunches] = useState<ExtendedPunch[]>(
    [...mockPunchLogs].reverse().slice(0, 20).map(p => ({ ...p, deviceName: mockDeviceList[Math.floor(Math.random() * mockDeviceList.length)].name }))
  );
  ...
  useEffect(() => {
    if (isLive) {
      intervalRef.current = setInterval(() => {
        const newPunch = generateLivePunch();
        setPunches(prev => [newPunch, ...prev].slice(0, 80));
        setStats(prev => ({
          total: prev.total + 1,
          checkIn: newPunch.verifyMode === "Check-In" ? prev.checkIn + 1 : prev.checkIn,
          checkOut: newPunch.verifyMode === "Check-Out" ? prev.checkOut + 1 : prev.checkOut,
          failed: newPunch.status === "failed" ? prev.failed + 1 : prev.failed,
        }));
        playBeep(newPunch.status === "failed");
      }, 2500);
    }
  ...
  useEffect(() => {
    heartbeatRef.current = setInterval(() => {
      const devId = mockDeviceList[Math.floor(Math.random() * mockDeviceList.length)].id;
      setDeviceHeartbeats(prev => ({ ...prev, [devId]: Date.now() }));
    }, 3000);
  ...
  const onlineDevices = mockDeviceList.filter(d => d.status === "online");
  const offlineDevices = mockDeviceList.filter(d => d.status === "offline");
      ...
      <div className="p-4 md:p-6" style={{ background: bg, minHeight: "100vh" }}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
          <div>
            ...
            <p className="text-sm mt-0.5" style={{ color: textSecondary }}>Real-time punch feed across all {mockDeviceList.length} devices</p>
          </div>
            ...
            <div className="rounded-xl overflow-hidden" style={{ background: cardBg, border: `1px solid ${border}` }}>
              <div className="px-4 py-3 flex items-center justify-between" style={{ borderBottom: `1px solid ${border}` }}>
                ...
                <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: "#dcfce7", color: "#16a34a" }}>
                  {onlineDevices.length}/{mockDeviceList.length}
                </span>
              </div>
              <div className="p-3 space-y-1.5 max-h-72 overflow-y-auto">
                {mockDeviceList.map(dev => {
                  ...
                  return (
                    <button
                      key={dev.id}
                      onClick={() => { setSelectedDevice(selectedDevice === dev.id ? null : dev.id); navigate(`/devices/${dev.id}`); }}
