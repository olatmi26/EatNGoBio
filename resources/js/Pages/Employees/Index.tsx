...
export default function EmployeesPage() {
  ...
  const [biometricAction, setBiometricAction] = useState<BiometricActionType | null>(null);
  ...
  const handleSave = (data: Partial<EmployeeItem>) => {
    ...
    if (editEmployee) {
      ...
      showToast("success", "Employee Added", `${data.firstName} ${data.lastName} registered successfully`);
    }
  };
  ...
  const handleDelete = (id: string) => {
    ...
    showToast("success", "Employee Removed", `${e?.firstName} ${e?.lastName} removed`);
  };

  const handleStatusChange = (id: string, newStatus: Employee["status"], label: string) => {
    const e = employees.find(x => x.id === id);
    setEmployees(prev => prev.map(x => x.id === id ? { ...x, status: newStatus } : x));
    showToast("success", label, `${e?.firstName} ${e?.lastName} ${label.toLowerCase()}`);
    setRowAction(null);
  };
  ...
  const handleTransferConfirm = (type: TransferType, value: string, _reason: string, _effectiveDate: string) => {
    ...
    if (type === "department") {
      ...
      setEmployees(prev => prev.map(e => targetIds.includes(e.id) ? { ...e, position: value } : e));
      showToast("success", "Position Transfer", `${targetIds.length} employee(s) moved to ${value}`);
    } else if (type === "area") {
      setEmployees(prev => prev.map(e => targetIds.includes(e.id) ? { ...e, area: value } : e));
      showToast("success", "Area Transfer", `${targetIds.length} employee(s) transferred to ${value}`);
    } else if (type === "probation") {
      setEmployees(prev => prev.map(e => targetIds.includes(e.id) ? { ...e, status: "active" } : e));
      showToast("success", "Probation Passed", `${targetIds.length} employee(s) confirmed as active`);
    } else if (type === "resignation") {
      setEmployees(prev => prev.map(e => targetIds.includes(e.id) ? { ...e, status: "resigned" } : e));
      showToast("success", "Resignation Processed", `${targetIds.length} employee(s) marked as resigned`);
    }
    setSelected([]);
  };
  ...
  const handleBiometricAction = (type: BiometricActionType) => {
    ...
    showToast("success", "Action Complete", `${labels[type]} ${count} employee(s)`);
    setSelected([]);
  };

  const toggleSelect = (id: string) => setSelected(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);
  const toggleAll = () => setSelected(prev => prev.length === paginated.length ? [] : paginated.map(e => String(e.id)));
  ...
  const DropdownMenu = ({ items }: { items: { label: string; action: () => void; danger?: boolean }[] }) => (
    <div className="absolute top-full left-0 mt-1 z-20 rounded-xl py-1 min-w-[200px]" style={{ background: dropdownBg, border: `1px solid ${border}`, boxShadow: "0 4px 20px rgba(0,0,0,0.12)" }}>
      {items.map(item => (
        <button key={item.label} onClick={() => { item.action(); setOpenDropdown(null); }}
          ...
          onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = item.danger ? "#fef2f2" : (isDark ? "#374151" : "#f9fafb"); }}
  ...
  const RowActionMenu = ({ emp }: { emp: Employee }) => (
    <div className="absolute right-0 top-full mt-1 z-30 rounded-xl py-1 min-w-[180px]"
      style={{ background: dropdownBg, border: `1px solid ${border}`, boxShadow: "0 8px 24px rgba(0,0,0,0.15)" }}>
      <button onClick={() => router.visit(`/employees/${emp.id}`)}
        ...
        onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "transparent"; }}>
        <i className="ri-eye-line text-sm" style={{ color: "#0891b2" }}></i> View Profile
      </button>
      <button onClick={() => { openEdit(emp); setRowAction(null); }}
        className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm cursor-pointer whitespace-nowrap"
        style={{ color: textPrimary }}
        onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = isDark ? "#374151" : "#f9fafb"; }}
        onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "transparent"; }}>
        <i className="ri-edit-line text-sm" style={{ color: "#16a34a" }}></i> Edit
      </button>
      <div style={{ height: "1px", background: border, margin: "4px 0" }}></div>
      {emp.status !== "suspended" && (
        <button onClick={() => handleStatusChange(emp.id, "suspended" as Employee["status"], "Suspended")}
          className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm cursor-pointer whitespace-nowrap"
          style={{ color: "#d97706" }}
      ...
      {emp.status === "suspended" && (
        <button onClick={() => handleStatusChange(emp.id, "active", "Reactivated")}
          className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm cursor-pointer whitespace-nowrap"
          style={{ color: "#16a34a" }}
      ...
      {emp.status !== "resigned" && (
        <button onClick={() => handleStatusChange(emp.id, "resigned", "Marked as Resigned")}
          className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm cursor-pointer whitespace-nowrap"
          style={{ color: "#dc2626" }}
          onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "#fef2f2"; }}
          onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "transparent"; }}>
          <i className="ri-logout-box-line text-sm"></i> Mark as Resigned
        </button>
      )}
      {emp.status !== "disabled" && (
        <button onClick={() => handleStatusChange(emp.id, "disabled" as Employee["status"], "Disabled")}
          className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm cursor-pointer whitespace-nowrap"
          style={{ color: "#6b7280" }}
      ...
      <div style={{ height: "1px", background: border, margin: "4px 0" }}></div>
      <button onClick={() => { setDeleteId(emp.id); setRowAction(null); }}
        className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm cursor-pointer whitespace-nowrap"
        style={{ color: "#dc2626" }}
        onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "#fef2f2"; }}
        onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "transparent"; }}>
    ...
    <AppLayout title="">
      <div className="p-4 md:p-6" style={{ background: bg, minHeight: "100vh" }} onClick={() => { setOpenDropdown(null); setRowAction(null); }}>
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium cursor-pointer whitespace-nowrap transition-colors" style={{ background: "linear-gradient(135deg, #16a34a, #15803d)", color: "#fff" }}>
            <i className="ri-add-line"></i> <span className="hidden sm:inline">Add Employee</span><span className="sm:hidden">Add</span>
          </button>

          {selected.length > 0 && (
            <button onClick={() => { selected.forEach(id => handleDelete(id)); setSelected([]); }} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium cursor-pointer whitespace-nowrap" style={{ background: "#fef2f2", color: "#dc2626", border: "1px solid #fecaca" }}>
          ...
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setOpenDropdown(openDropdown === "import" ? null : "import")} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium cursor-pointer whitespace-nowrap" style={{ background: isDark ? "#374151" : "#f3f4f6", color: textPrimary, border: `1px solid ${border}` }}>
              Import <i className="ri-arrow-down-s-line text-xs"></i>
            </button>
            {openDropdown === "import" && (
              <DropdownMenu items={[
                { label: "Import Employee", action: () => showToast("info", "Import Employee", "Select a CSV/Excel file to import") },
                { label: "Import Document", action: () => showToast("info", "Import Document", "Upload employee documents") },
                { label: "Import Photo", action: () => showToast("info", "Import Photo", "Upload employee photos") },
                { label: "Import USB Employee", action: () => showToast("info", "Import USB", "Connect USB device to import") },
              ]} />
            )}
          </div>
          ...
          <div className="relative hidden md:block" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setOpenDropdown(openDropdown === "transfer" ? null : "transfer")} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium cursor-pointer whitespace-nowrap" style={{ background: isDark ? "#374151" : "#f3f4f6", color: textPrimary, border: `1px solid ${border}` }}>
              Transfer <i className="ri-arrow-down-s-line text-xs"></i>
            </button>
            {openDropdown === "transfer" && (
              <DropdownMenu items={[
                { label: "Department Transfer", action: () => setTransferType("department") },
                { label: "Position Transfer", action: () => setTransferType("position") },
                { label: "Area Transfer", action: () => setTransferType("area") },
                { label: "Pass Probation", action: () => setTransferType("probation") },
                { label: "Resignation", action: () => setTransferType("resignation"), danger: true },
              ]} />
            )}
          </div>
          ...
          {/* More dropdown */}
          <div className="relative hidden md:block" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setOpenDropdown(openDropdown === "more" ? null : "more")} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium cursor-pointer whitespace-nowrap" style={{ background: isDark ? "#374151" : "#f3f4f6", color: textPrimary, border: `1px solid ${border}` }}>
              More <i className="ri-arrow-down-s-line text-xs"></i>
            </button>
            {openDropdown === "more" && (
              <DropdownMenu items={[
                { label: "Resynchronize to Device", action: () => setBiometricAction("resync") },
                { label: "Re-upload from Device", action: () => setBiometricAction("reupload") },
                { label: "Delete Biometric Template", action: () => setBiometricAction("delete-template"), danger: true },
                { label: "Export USB Employee", action: () => setBiometricAction("export-usb") },
              ]} />
            )}
          </div>
              ...
              <thead>
                <tr style={{ borderBottom: `1px solid ${border}` }}>
                  <th className="px-4 py-3 w-10">
                    <input type="checkbox" checked={selected.length === paginated.length && paginated.length > 0} onChange={toggleAll} className="cursor-pointer" />
                  </th>
                  {["Employee ID", "Name", "Department", "Position", "Area", "Type", "Status", "Actions"].map(h => (
              ...
              <tbody>
                {paginated.map((e) => (
                  <tr key={e.id} style={{ borderBottom: `1px solid ${border}` }}
                    onMouseEnter={(el) => { (el.currentTarget as HTMLTableRowElement).style.background = isDark ? "#374151" : "#f9fafb"; }}
                    onMouseLeave={(el) => { (el.currentTarget as HTMLTableRowElement).style.background = "transparent"; }}>
                    <td className="px-4 py-3">
                      <input type="checkbox" checked={selected.includes(String(e.id))} onChange={() => toggleSelect(String(e.id))} className="cursor-pointer" />
                    </td>
                    <td className="px-4 py-3">
                      <button onClick={() => router.visit(`/employees/${e.id}`)} className="text-sm font-medium cursor-pointer hover:underline whitespace-nowrap" style={{ color: "#16a34a" }}>{e.employeeId}</button>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0" style={{ background: getAvatarColor(e.id) }}>
                          {getInitials(e)}
                        </div>
                        <div>
                          <p className="text-sm font-medium whitespace-nowrap" style={{ color: textPrimary }}>{e.firstName} {e.lastName}</p>
                          <p className="text-xs" style={{ color: textSecondary }}>{e.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-sm whitespace-nowrap" style={{ color: textPrimary }}>{e.department}</td>
                    <td className="px-4 py-3 text-sm whitespace-nowrap" style={{ color: textSecondary }}>{e.position}</td>
                    <td className="px-4 py-3 text-sm whitespace-nowrap" style={{ color: textSecondary }}>{e.area}</td>
                    <td className="px-4 py-3 text-sm whitespace-nowrap" style={{ color: textSecondary }}>{e.employmentType}</td>
                    <td className="px-4 py-3">{statusBadge(e.status)}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => router.visit(`/employees/${e.id}`)}
                          className="w-7 h-7 flex items-center justify-center rounded-lg cursor-pointer"
                          style={{ color: "#0891b2" }} title="View Profile"
                          onMouseEnter={(el) => { (el.currentTarget as HTMLButtonElement).style.background = "#e0f2fe"; }}
                          onMouseLeave={(el) => { (el.currentTarget as HTMLButtonElement).style.background = "transparent"; }}>
                          <i className="ri-eye-line text-sm"></i>
                        </button>
                        <button onClick={() => openEdit(e as Employee)}
                          className="w-7 h-7 flex items-center justify-center rounded-lg cursor-pointer"
                          style={{ color: "#16a34a" }} title="Edit Employee"
                          onMouseEnter={(el) => { (el.currentTarget as HTMLButtonElement).style.background = "#dcfce7"; }}
                          onMouseLeave={(el) => { (el.currentTarget as HTMLButtonElement).style.background = "transparent"; }}>
                          <i className="ri-edit-line text-sm"></i>
                        </button>
                        <div className="relative" onClick={(ev) => ev.stopPropagation()} ref={rowAction === String(e.id) ? rowActionRef : null}>
                          <button
                            onClick={() => setRowAction(rowAction === String(e.id) ? null : String(e.id))}
                            className="w-7 h-7 flex items-center justify-center rounded-lg cursor-pointer"
                            style={{ color: textSecondary }} title="More Actions"
                            onMouseEnter={(el) => { (el.currentTarget as HTMLButtonElement).style.background = isDark ? "#4b5563" : "#f3f4f6"; }}
                            onMouseLeave={(el) => { (el.currentTarget as HTMLButtonElement).style.background = "transparent"; }}>
                            <i className="ri-more-2-line text-sm"></i>
                          </button>
                          {rowAction === String(e.id) && <RowActionMenu emp={e as Employee} />}
                        </div>
                      </div>
                    </td>
                  </tr>
      ...
      {biometricAction && (
        <BiometricActionModal open={!!biometricAction} onClose={() => setBiometricAction(null)} type={biometricAction} employees={employees} selectedIds={selected} onConfirm={handleBiometricAction} />
      )}
    </AppLayout>
  );
}
