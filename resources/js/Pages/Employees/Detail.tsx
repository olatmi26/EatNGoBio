...
// Add or define Props type if not already present elsewhere in codebase
type Props = {
  employee: any;
  recentLogs: any[];
  heatmap?: { year: number; month: number; data: any[]; }; // now optional
...
const generateHeatmapData = (employeeId: string) => {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (date > today) {
      ...
      // Use first character if available, else fallback to 0
      const firstChar = String(employeeId ?? "0")[0] ?? "0";
      const rand = (firstChar.charCodeAt(0) + d) % 10;
...
// This function expects props to be passed directly from the parent (like <EmployeeDetailPage {...page.props} />)
export default function EmployeeDetailPage(props: Props) {
  // id comes from Inertia route — extract from props
  const { isDark } = useTheme();
  const { showToast } = useToast();
  const [activeTab, setActiveTab] = useState<TabKey>('profile');
  const [confirmAction, setConfirmAction] = useState<string | null>(null);

  const { employee, recentLogs, heatmap: _heatmap, connectedDevices } = props;
    ...
    return (
      <AppLayout title="">
        <div className="p-6 flex flex-col items-center justify-center min-h-[60vh]">
          ...
          <button onClick={() => router.visit('/employees')} className="mt-4 px-4 py-2 rounded-lg text-sm font-medium text-white cursor-pointer" style={{ background: '#16a34a' }}>Back to Employees</button>
        </div>
      </AppLayout>
    );
  }
  ...
  // More robust hashing for avatar color
  const avatarColors = ['#16a34a', '#0891b2', '#7c3aed', '#d97706', '#dc2626', '#db2777'];
  let avatarIndex = 0;
  if (employee && employee.id != null) {
    const empIdString = typeof employee.id === 'string' ? employee.id : employee.id.toString ? employee.id.toString() : "";
    if (empIdString.length > 0) {
      avatarIndex = String(empIdString).charCodeAt(0) % avatarColors.length;
    }
  }
  ...
  // Defensive: Make sure we have a heatmap. Generate if not.
  let heatmap = _heatmap;
  if (!heatmap || !Array.isArray(heatmap.data)) {
    // Auto-generate dummy heatmap for safe fallback with valid structure
    heatmap = generateHeatmapData(employee?.employeeId || employee?.id || "0");
  }

  const presentDays = Array.isArray(heatmap.data)
    ? heatmap.data.filter(d => d.status === 'present').length
    : 0;
  const lateDays = Array.isArray(heatmap.data)
    ? heatmap.data.filter(d => d.status === 'late').length
    : 0;
  const absentDays = Array.isArray(heatmap.data)
    ? heatmap.data.filter(d => d.status === 'absent').length
    : 0;
  const workDays = Array.isArray(heatmap.data)
    ? heatmap.data.filter(d => d.status !== 'weekend' && d.status !== 'future').length
    : 0;
  const attendanceRate =
    workDays > 0 ? Math.round(((presentDays + lateDays) / workDays) * 100) : 0;
  ...
  const handleBiometricAction = (key: string, danger?: boolean) => {
    ...
    showToast(
      'success',
      biometricActions.find(a => a.key === key)?.label || key,
              ...
              <table className="w-full">
                ...
                <tbody>
                  {connectedDevices.map(dev => (
                    ...
                    >
                      <td className="px-4 py-3">
                        <button onClick={() => router.visit(`/devices/${dev.id}`)} className="text-sm font-medium cursor-pointer hover:underline" style={{ color: '#16a34a' }}>{dev.name}</button>
                      </td>
