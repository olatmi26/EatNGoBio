...
// Convert a string array to objects with mock device/employee counts for display UI compatibility
function getAreaObjects(areas: string[] | undefined) {
  if (!areas) return [];
  return areas.map((name, idx) => ({
    id: idx + 1,
    name,
    devices: Math.floor(Math.random() * 4) + 1, // mock device number 1-4
    employees: Math.floor(Math.random() * 20) + 2, // mock employee number 2-21
  }));
}
...
function LocationSearchSelect({
  ...
  const filtered = areaOptions.filter(a =>
