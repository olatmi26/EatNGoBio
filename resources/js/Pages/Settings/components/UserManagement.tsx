...
export default function UserManagement({ 
  ...
  const handleSaveUser = () => {
    if (!userForm.name || !userForm.email || (!editUser && !userForm.password) || !userForm.role) {
    ...
    const url = editUser ? `/settings/users/${editUser.id}` : '/settings/users';
    ...
    router[method](url, userForm, {
      ...
      onSuccess: () => {
        ...
        showToast("success", editUser ? "User Updated" : "User Created", 
          `${userForm.name} has been ${editUser ? 'updated' : 'created'} successfully`);
        setIsSubmitting(false);
      },
      onError: (errors) => {
        const errorMsg = Object.values(errors).flat().join(', ');
        showToast("error", "Save Failed", errorMsg || "Could not save user");
        setIsSubmitting(false);
      }
    });
  };
  ...
  const handleDeleteUser = () => {
    ...
    router.delete(`/settings/users/${deleteUserId}`, {
      preserveScroll: true,
      onSuccess: () => {
        setUsers(prev => prev.filter(u => u.id !== deleteUserId));
        setDeleteUserId(null);
        showToast("success", "User Removed", "User has been removed from the system");
      },
      onError: (errors) => {
        showToast("error", "Delete Failed", errors.message || "Could not delete user");
        setDeleteUserId(null);
      }
    });
  };
  ...
  const handleSaveRole = () => {
    ...
    const url = editRole ? `/settings/roles/${editRole.id}` : '/settings/roles';
    ...
    router[method](url, roleForm, {
      ...
      onSuccess: () => {
        ...
        showToast("success", editRole ? "Role Updated" : "Role Created", 
          `${roleForm.name} has been ${editRole ? 'updated' : 'created'} successfully`);
        setIsSubmitting(false);
      },
      onError: (errors) => {
        const errorMsg = Object.values(errors).flat().join(', ');
        showToast("error", "Save Failed", errorMsg || "Could not save role");
        setIsSubmitting(false);
      }
    });
  };
  ...
  const handleDeleteRole = () => {
    ...
    router.delete(`/settings/roles/${deleteRoleId}`, {
      preserveScroll: true,
      onSuccess: () => {
      ...
      onError: (errors) => {
        showToast("error", "Delete Failed", errors.message || "Cannot delete this role");
        setDeleteRoleId(null);
      }
    });
  };
  ...
  const selectAllCategory = (category: string) => {
    ...
    if (allSelected) {
      setRoleForm(f => ({ 
        ...f, 
        permissions: f.permissions.filter(k => !catPerms.includes(k)) 
      }));
    } else {
      setRoleForm(f => ({ 
        ...f, 
        permissions: [...new Set([...f.permissions, ...catPerms])] 
      }));
    }
  };
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ background: cardBg, border: `1px solid ${border}` }}>
      ...
      <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: `1px solid ${border}` }}>
        ...
        <div className="flex items-center gap-2">
          {subTab === 'users' && (
            <button 
              onClick={openAddUser} 
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium cursor-pointer whitespace-nowrap text-white hover:opacity-90 transition-opacity" 
      ...
      <div className="flex gap-1 px-6 pt-3" style={{ borderBottom: `1px solid ${border}` }}>
        {(['users', 'roles'] as SubTab[]).map(key => (
          <button 
            key={key} 
            onClick={() => setSubTab(key)}
            className="flex items-center gap-2 px-4 py-2 text-xs font-medium cursor-pointer whitespace-nowrap transition-colors"
            style={{ 
              color: subTab === key ? '#16a34a' : textSecondary, 
              borderBottom: subTab === key ? '2px solid #16a34a' : '2px solid transparent' 
            }}
          >
            <i className={key === 'users' ? 'ri-team-line' : 'ri-shield-check-line'}></i>
              ...
              {users.map(u => {
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0" 
                             style={{ background: role?.color || '#16a34a' }}>
                          {u.name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}
                        </div>
                        <span className="text-sm font-medium whitespace-nowrap" style={{ color: textPrimary }}>
                          {u.name}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-3">
                      <span className="px-2.5 py-1 rounded-full text-xs font-medium whitespace-nowrap" 
                            style={{ background: `${role?.color || '#16a34a'}20`, color: role?.color || '#16a34a' }}>
                        {u.roleName}
                      </span>
                    </td>
                    <td className="px-5 py-3">{statusBadge(u.status)}</td>
                    <td className="px-5 py-3 text-xs font-mono whitespace-nowrap" style={{ color: textSecondary }}>
                      {u.lastLogin}
                    </td>
                    <td className="px-5 py-3 text-xs whitespace-nowrap" style={{ color: textSecondary }}>
                      {u.createdAt}
                    </td>
                    <td className="px-5 py-3">
            ...
            <div key={role.id} className="p-4 rounded-xl" 
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {role.isSystem && (
                        <span className="px-2 py-0.5 rounded-full text-xs font-medium" 
                              style={{ background: '#fef9c3', color: '#ca8a04' }}>System</span>
                      )}
                      <span className="px-2 py-0.5 rounded-full text-xs font-medium" 
              ...
              <div className="flex flex-wrap gap-1.5 mt-3">
                {role.permissions.slice(0, 8).map(pKey => {
                  ...
                  return perm ? (
                    <span key={pKey} className="px-2 py-0.5 rounded text-xs" 
                          style={{ background: isDark ? '#4b5563' : '#e5e7eb', color: textSecondary }}>
                      {perm.label}
                    </span>
                  ) : null;
                })}
          ...
          <div className="w-full max-w-md rounded-2xl shadow-xl" 
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <select 
                  ...
                  onChange={(e) => setUserForm(f => ({ ...f, status: e.target.value as SystemUser['status'] }))} 
                  className={inputClass} 
                  style={inputStyle}
                >
            ...
            <div className="px-6 py-4 flex items-center justify-end gap-3" 
              ...
              <button 
                ...
                disabled={isSubmitting || !userForm.name || !userForm.email || (!editUser && !userForm.password) || !userForm.role}
                className="px-5 py-2 text-sm rounded-lg cursor-pointer whitespace-nowrap font-semibold text-white disabled:opacity-50 disabled:cursor-not-allowed hover:opacity-90 transition-opacity"
                style={{ background: '#16a34a' }}
              >
                {isSubmitting ? (
                  <>
      ...
      {showRoleModal && (
        ...
        >
          <div className="w-full max-w-2xl rounded-2xl flex flex-col max-h-[90vh] shadow-xl" 
               style={{ background: isDark ? '#1f2937' : '#ffffff', border: `1px solid ${border}` }}>
            <div className="flex items-center justify-between px-6 py-4" 
                 style={{ borderBottom: `1px solid ${border}` }}>
              <h3 className="text-base font-semibold" style={{ color: textPrimary }}>
                  ...
                  <div className="flex gap-2 mt-1 flex-wrap">
                    {ROLE_COLORS.map(c => (
                      <button 
                        ...
                        style={{ 
                          background: c, 
                          border: roleForm.color === c ? `3px solid ${isDark ? '#fff' : '#111'}` : '3px solid transparent' 
                        }}
                      >
                        {roleForm.color === c && (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <div className="w-4 h-4 rounded flex items-center justify-center" 
                                 style={{ 
                                   background: allSelected ? '#16a34a' : someSelected ? '#dcfce7' : 'transparent', 
                                   border: `1.5px solid ${allSelected ? '#16a34a' : someSelected ? '#16a34a' : border}` 
                                 }}>
                              {allSelected && <i className="ri-check-line text-white" style={{ fontSize: '10px' }}></i>}
                              {someSelected && !allSelected && (
                                <div className="w-2 h-0.5 rounded" style={{ background: '#16a34a' }}></div>
                              )}
                            </div>
                          ...
                          {perms.map(perm => (
                            ...
                            >
                              <div className="w-4 h-4 rounded flex items-center justify-center flex-shrink-0" 
                                   style={{ 
                                     background: roleForm.permissions.includes(perm.key) ? '#16a34a' : 'transparent', 
                                     border: `1.5px solid ${roleForm.permissions.includes(perm.key) ? '#16a34a' : border}` 
                                   }}>
      ...
      <ConfirmDialog 
        ...
        title="Remove User" 
        message="Are you sure you want to remove this user? They will lose all system access." 
        confirmLabel="Remove" 
        danger 
      />
      <ConfirmDialog 
        ...
        title="Delete Role" 
        message="Deleting this role will affect all users assigned to it. Are you sure?" 
        confirmLabel="Delete" 
        danger 
      />
    </div>
