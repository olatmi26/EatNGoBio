...
import type { PageProps, DeviceItem, PunchLog, SyncHistory, DeviceCommandItem } from '@/types';
...
export default function DeviceDetailPage() {
  ...
  const { device, punchLogs: allPunchLogs, syncHistory: allSyncHistory, commands: initialCommands, connectedEmployees } = props;
  const [activeTab, setActiveTab] = useState<TabKey>('overview');
  const [commands, setCommands] = useState<DeviceCommandItem[]>(initialCommands);
  const [punchSearch, setPunchSearch] = useState('');

  const devicePunches = allPunchLogs.filter(p =>
  ...
  const statusConfig: Record<DeviceItem['status'], { bg: string; color: string; icon: string; label: string; dot: string }> = {
  ...
  const getAvatarColor = (id: string | number) => { const s = String(id); return avatarColors[s.length > 0 ? s.charCodeAt(0) % avatarColors.length : 0]; };
    ...
    <AppLayout title="">
      <div className="p-6" style={{ background: bg, minHeight: '100vh' }}>
        ...
        <div className="flex items-center gap-2 mb-5 text-sm">
          <button onClick={() => router.visit('/devices')} className="cursor-pointer hover:underline" style={{ color: '#16a34a' }}>Devices</button>
        ...
        {activeTab === 'sync' && (
          <div className="rounded-xl overflow-hidden" style={{ background: cardBg, border: `1px solid ${border}` }}>
            ...
            <table className="w-full">
              ...
              <tbody>
                {(allSyncHistory ?? []).map(sh => { if (!sh) return null;
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0" style={{ background: getAvatarColor(String(emp.id ?? "")) }}>
