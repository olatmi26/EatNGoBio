...
export default function DevicesPage() {
  ...
  const handleSave = () => {
    ...
    if (editDevice) {
      router.put(route('devices.update', editDevice.id, false, props.ziggy), form, { onSuccess: () => { setShowModal(false); showToast('success', 'Device Updated', ''); } });
    } else {
      router.post(route('devices.store', undefined, false, props.ziggy), form, { onSuccess: () => { setShowModal(false); showToast('success', 'Device Added', ''); } });
    }
  };
  ...
  const handleDelete = (id: number) => {
    ...
    router.delete(route('devices.destroy', id, false, props.ziggy), {
      ...
      onSuccess: () => {
        ...
        showToast('success', 'Device Removed', name ? `${name} has been removed` : 'Device removed.');
      },
      onError: () => {
  ...
  const bulkDeleteSelected = () => {
    ...
    const run = (index: number) => {
      if (index >= ids.length) {
        showToast('success', 'Devices removed', `${ids.length} device(s) removed.`);
        setSelected([]);
        return;
      }
      router.delete(route('devices.destroy', ids[index], false, props.ziggy), {
        preserveScroll: true,
        onSuccess: () => run(index + 1),
        onError: () => {
          showToast('error', 'Error', 'Could not remove one or more devices.');
          setSelected([]);
        },
      });
    };
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {[
            ...
            <div key={s.label} className="p-5 rounded-xl" style={{ background: cardBg, border: `1px solid ${border}` }}>
              ...
              <p className="text-3xl font-bold" style={{ color: s.color }}>{Number.isFinite(s.value) ? s.value : 0}</p>
            </div>
          ))}
        </div>
              ...
              <tbody>
                {filtered.map((d) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => router.visit(`/devices/${d.id}`)} className="text-sm font-medium cursor-pointer hover:underline whitespace-nowrap" style={{ color: '#16a34a' }}>
                        {d.name}
                      </button>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={() => { router.visit(`/devices/${d.id}`); }} className="w-7 h-7 flex items-center justify-center rounded-lg cursor-pointer transition-colors" style={{ color: '#7c3aed' }} title="Command Center"
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              type="number"
              value={Number.isFinite(form.heartbeat) ? form.heartbeat : emptyForm.heartbeat}
              onChange={(e) => {
                const n = Number(e.target.value);
                setForm(f => ({ ...f, heartbeat: e.target.value === '' || !Number.isFinite(n) ? emptyForm.heartbeat : n }));
              }}
