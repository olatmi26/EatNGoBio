...
export default function ShiftModal({ open, onClose, onSave, editShift, areas = [] }: Props) {
  ...
  /* ── Location helpers ── */
  const allSelected = areas.length > 0 && selectedLocations.length === areas.length;
          ...
          {activeTab === 'basic' && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label style={labelStyle}>
                    ...
                    <span
                      ...
                      style={{ background: '#dcfce7', color: '#16a34a', fontSize: '10px' }}
                    >
                      earliest allowed
                    </span>
                  </label>
                  <input
                    ...
                    onChange={(e) => setForm(f => ({ ...f, checkinStartAt: e.target.value }))}
                    className={inputClass}
                    style={inputStyle}
                  />
                  <p className="text-xs mt-1" style={{ color: textSecondary }}>
                    System won't accept clock-ins before this time
                  </p>
                </div>
                ...
                <div>
                  <label style={labelStyle}>
                    ...
                    <span
                      className="ml-1 px-1.5 py-0.5 rounded text-xs"
                      style={{ background: '#fef9c3', color: '#ca8a04', fontSize: '10px' }}
                    >
                      latest valid
                    </span>
                  </label>
                  <input
                    ...
                    onChange={(e) => setForm(f => ({ ...f, checkoutEndsAt: e.target.value }))}
                    className={inputClass}
                    style={inputStyle}
                  />
                  <p className="text-xs mt-1" style={{ color: textSecondary }}>
                    Checkouts after this time are flagged as missed
                  </p>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setForm(f => ({ ...f, lateThreshold: Number(e.target.value) }))}
                    className={inputClass}
                    style={inputStyle}
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setForm(f => ({ ...f, overtimeThreshold: Number(e.target.value) }))}
                    className={inputClass}
                    style={inputStyle}
                  />
                </div>
              ...
              <div
                ...
                style={{ background: isDark ? '#374151' : '#f0fdf4', border: `1px solid ${isDark ? '#4b5563' : '#bbf7d0'}` }}
              >
                <div className="flex items-center gap-1.5" style={{ color: '#16a34a' }}>
                  <i className="ri-login-circle-line"></i>
                  <span>Check-in window: <strong>{form.checkinStartAt || '07:30'}</strong> → <strong>{form.startTime || '08:00'}</strong></span>
                </div>
                <div className="w-px h-4" style={{ background: '#bbf7d0' }}></div>
                <div className="flex items-center gap-1.5" style={{ color: '#ca8a04' }}>
                  <i className="ri-logout-circle-r-line"></i>
                  <span>Checkout window: <strong>{form.endTime || '17:00'}</strong> → <strong>{form.checkoutEndsAt || '18:00'}</strong></span>
                </div>
              </div>
                ...
                <div className="flex gap-2 mt-1">
                  {COLORS.map(c => (
                    <button
                      ...
                      style={{
                        background: c,
                        border: selectedColor === c ? `3px solid ${isDark ? '#fff' : '#111'}` : '3px solid transparent',
                      }}
                    >
                ...
                {DAYS.map((day, idx) => (
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        background: workdays[idx] ? selectedColor : 'transparent',
            ...
            <div className="space-y-3">
              ...
              {breaks.length === 0 && (
                ...
                >
                  ...
                  <button onClick={addBreak} className="mt-2 text-xs cursor-pointer" style={{ color: '#16a34a' }}>
                    Add a break time
                  </button>
                </div>
              )}
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <div>
                      ...
                      <button
                        ...
                        style={{
                          background: brk.paid ? '#dcfce7' : (isDark ? '#4b5563' : '#f3f4f6'),
                          color: brk.paid ? '#16a34a' : textSecondary,
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <span
                  ...
                  style={{ background: '#dcfce7', color: '#16a34a' }}
                >
                  {selectedLocations.length} / {areas.length} selected
                </span>
              </div>

              <p className="text-xs" style={{ color: textSecondary }}>
                This shift will be available for employees in the selected locations
              </p>
              ...
              {areas.length === 0 ? (
                <div
                  ...
                  style={{ background: isDark ? '#374151' : '#f9fafb', border: `1px dashed ${border}` }}
                >
                  <i className="ri-map-pin-line text-3xl mb-2 block" style={{ color: textSecondary }}></i>
                  <p className="text-sm" style={{ color: textSecondary }}>No locations found</p>
                  <p className="text-xs mt-1" style={{ color: textSecondary, opacity: 0.7 }}>
                    Add locations in the Locations module first
                  </p>
                </div>
              ) : (
                <>
                  ...
                  <div
                    ...
                    style={{
                      background: allSelected ? '#dcfce7' : (isDark ? '#374151' : '#f9fafb'),
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <p className="text-xs font-semibold" style={{ color: allSelected ? '#16a34a' : textPrimary }}>
                        All Locations
                      </p>
                      <p className="text-xs" style={{ color: textSecondary }}>
                        {allSelected ? 'Click to deselect all' : someSelected ? `${selectedLocations.length} selected — click to select all` : 'Click to select all locations'}
                      </p>
                    </div>
                    {allSelected && <i className="ri-check-double-line flex-shrink-0" style={{ color: '#16a34a' }}></i>}
                  </div>

                  {/* Divider */}
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-px" style={{ background: border }}></div>
                    <span className="text-xs" style={{ color: textSecondary }}>individual locations</span>
                    <div className="flex-1 h-px" style={{ background: border }}></div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {areas.map(areaName => {
                      const isSelected = selectedLocations.includes(areaName);
                      return (
                        <div
                          key={areaName}
                          className="flex items-center gap-2.5 px-3 py-3 rounded-xl cursor-pointer transition-all"
                          style={{
                            background: isSelected ? '#dcfce7' : (isDark ? '#374151' : '#f9fafb'),
                            border: `1px solid ${isSelected ? '#16a34a' : border}`,
                          }}
                          onClick={() => toggleLocation(areaName)}
                        >
                          <input
                            type="checkbox"
                            checked={isSelected}
                          ...
                          <div className="flex items-center gap-2 min-w-0 flex-1">
                            <div
                              ...
                              style={{ background: isSelected ? '#16a34a' : (isDark ? '#6b7280' : '#d1d5db') }}
                            ></div>
                            <p
                              className="text-xs font-medium truncate"
                              style={{ color: isSelected ? '#16a34a' : textPrimary }}
                            >
                              {areaName}
                            </p>
                          </div>
