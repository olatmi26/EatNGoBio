...
class EmployeeController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        if ($d = $request->input('department')) $query->where('department', $d);
        if ($a = $request->input('area'))       $query->where('area', $a);
        if ($st = $request->input('status') and $st !== 'all') $query->where('employee_status', $st);

        $perPage   = min((int) $request->input('per_page', 25), 100);
        $employees = $query->orderBy('first_name')->paginate($perPage)->through(fn($emp) => [
            'id'             => $emp->id,
    ...
    public function show(int $id): Response
    {
        ...
        // Last 30 days attendance — one query
        $from    = Carbon::today()->subDays(29);
        $to      = Carbon::today();
        $logRows = $emp->attendanceLogs()
        ...
        // Heatmap — one query
        $logsByDate = $emp->attendanceLogs()
            ->whereBetween('punch_time', [$from, $to])
            ->where('punch_type', 0)
            ->selectRaw('DATE(punch_time) as day')
            ->pluck('day')
            ->flip()
            ->map(fn() => 1)
            ->toArray();
        ...
        for ($d = $from->copy(); $d->lte($to); $d->addDay()) {
            $heatmap[$d->toDateString()] = isset($logsByDate[$d->toDateString()]) ? 1 : 0;
        }

        // Connected devices
        $connectedDevices = Device::with('location')
            ->where('approved', true)
            ->where('area', $emp->area)
            ->get()
            ->map(fn($dev) => [
    ...
    private function syncFKs(array &$data): void
    {
        ...
        if (!empty($data['position'])) {
            ...
            if ($pos) $data['position_id'] = $pos->id;
        }
    }
}
